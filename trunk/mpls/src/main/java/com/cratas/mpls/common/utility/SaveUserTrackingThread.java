...
public class SaveUserTrackingThread implements Runnable{
	
	    private final static Logger logger = LoggerFactory.getLogger(SaveUserTrackingThread.class);
		...
		public void setData(UserTrackingRequestDTO campaignUserDTO){
			   this.userTrackingDTO = campaignUserDTO;
		}
		...
		public void run() {
			   try {
					if(userTrackingDTO.getUrlType().equals(Constants.SINGLE_PATH)){
						 userService.addUserTrackingDetail(userTrackingDTO);
					}
			   } catch (Exception e) {
					logger.error("error in store user tracking data => "+e);
			   }
		}
}
