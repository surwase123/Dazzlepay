                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">Event Type</h5>
                            </div>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}"><i class="feather icon-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}/eventType/view">Event Type</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                     ...
                     <div class="card-body">
                           ...
                           <c:if test="${is_success == 'success'}">
                            ...
                            <c:if test="${action != 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/eventType/add" var="actionUrl"/>
                            </c:if>
                            <c:if test="${action == 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/eventType/edit" var="actionUrl"/>
                            </c:if>
                            <form id="eventType-form" action="${actionUrl}" method="post">
								<div class="col-md-12">
	                                <div class="form-group">
	                                    <label class="form-label">Event Type Name</label>
	                                    <select class="form-control js-example-basic-single" name="eventTypeName" id="eventTypeName">
	                                        <option value="">--Select Event Type Name--</option>
	                                        <c:forEach items="${eventTypeSysList}" var="eventTypeSys" varStatus="index">
	                                              <option value="${eventTypeSys}" <c:if test="${eventTypeSys == eventType.eventTypeName}">selected</c:if>>${eventTypeSys}</option>
	                                        </c:forEach>
	                                    </select>
	                                </div>
                                </div>
								<div class="col-md-12">
									<div class="form-group">
										...
										<textarea name="description" id="description" class="width">${eventType.description}</textarea>
									</div>
								</div>
								...
								<input type="hidden" name="id" value="<c:if test='${!empty(eventType.id)}'>${eventType.id}</c:if><c:if test='${empty(eventType.id)}'>0</c:if>">
                                                ...
                                                <c:if test="${pageRole.isUpdate == 1 || pageRole.isDelete == 1}">
                                                	<td>
	                                                     <c:if test="${pageRole.isUpdate == 1}">
	                                                     	<a href="javascript:updateEventType('${eventType.id}')"><i class="feather icon-edit"></i></a>
	                                                     </c:if>
	                                                     <c:if test="${pageRole.isDelete == 1}">
	                                                     	<a href="javascript:deleteEventType('${eventType.id}', '${sessionScope.loginDTO.loginId}')"><i class="feather icon-trash-2"></i></a>
	                                                     </c:if>
                                                 	</td>
                                                </c:if>
                                            </tr>
...
<script src="${applicationScope['jsBaseUrl']}/javascript/event/eventType.js" defer onload="loadPageScript()"></script>
</body>
</html>