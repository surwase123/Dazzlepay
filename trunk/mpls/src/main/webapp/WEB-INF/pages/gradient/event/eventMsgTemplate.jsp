        ...
        <div class="pcoded-content">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">Event Message Template</h5>
                            </div>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}"><i class="feather icon-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}/eventMsgTemplate/view">Event Message Template</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <c:if test="${action != 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/eventMsgTemplate/add" var="actionUrl"/>
                            </c:if>
                            <c:if test="${action == 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/eventMsgTemplate/edit" var="actionUrl"/>
                            </c:if>
                            <form id="eventMsgTemplate-form" action="${actionUrl}" method="post" onsubmit="return validateEventMsgTemplate('${action}', 'eventId')">
                            		<div class="col-md-12">
		                                <div class="form-group">
		                                    <label class="form-label">Event Source</label>
		                                    <select class="form-control js-example-basic-single" name="eventSourceId" id="eventSourceId" onchange="getEventBySource('eventSourceId', 'eventId')">
		                                        <option value="">--Select Event Source--</option>
		                                        <c:forEach items="${eventSourceList}" var="eventSource" varStatus="index">
		                                              <option value="${eventSource.id}" <c:if test="${eventSource.id == eventMsgTemplate.eventSourceId}">selected</c:if>>${eventSource.sourceName}</option>
		                                        </c:forEach>
		                                    </select>
		                                </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control js-example-basic-single" name="eventId" id="eventId" onchange="getEventParameters('eventId', 'eventParameters')" <c:if test="${action == 'update'}"> disabled="true"</c:if>>
                                                <option value="">--Select Event--</option>
                                                <c:forEach items="${eventList}" var="event" varStatus="index">
                                                      <option value="${event.id}" <c:if test="${event.id == eventMsgTemplate.eventId}">selected</c:if>>${event.eventName}</option>
                                                </c:forEach>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-row">
                                        <div class="form-group col-md-10">
                                            ...
                                            <textarea name="msgTemplate" id="msgTemplate" class="width">${eventMsgTemplate.msgTemplate}</textarea>
                                        </div>
                                        <div class="form-group col-md-2 draggable">
                                            <label class="form-label">Event Parameter</label>
                                            <select class="form-control js-example-basic-single" name="eventParameters" id="eventParameters">
                                                <option value="">--Select Event Parameter--</option>
                                            </select>
                                    ...
                                    <input type="hidden" name="id" value="<c:if test='${!empty(eventMsgTemplate.id)}'>${eventMsgTemplate.id}</c:if><c:if test='${empty(eventMsgTemplate.id)}'>0</c:if>">
                   ...
                   <c:if test="${!empty(eventMsgTemplateList)}">
                    <div class="card">
                        <div class="card-header">
                            <h5>Event Message Template</h5>
                        </div>
                        <div class="card-body">
                            <div class="dt-responsive table-responsive">
                                <table id="multi-colum-dt" class="table table-striped table-bordered nowrap">
                                    ...
                                    <tbody>
                                         <c:forEach items="${eventMsgTemplateList}" var="eventMsgTemplate" varStatus="index">
                                            <tr>
                                                <td>${index.count}</td>
                                                <td>${eventMsgTemplate.sourceName}</td>
                                                <td>${eventMsgTemplate.eventName}</td>
           		                                <td>${eventMsgTemplate.msgTemplate}</td>
                                                <c:if test="${pageRole.isUpdate == 1 || pageRole.isDelete == 1}">
                                                    <td>
                                                        <c:if test="${pageRole.isUpdate == 1}">
                                                            <a href="javascript:updateEventMsgTemplate('${eventMsgTemplate.id}')"><i class="feather icon-edit"></i></a>
                                                        </c:if>
                                                        <c:if test="${pageRole.isDelete == 1}">
                                                            <a href="javascript:deleteEventMsgTemplate('${eventMsgTemplate.id}', '${sessionScope.loginDTO.loginId}')"><i class="feather icon-trash-2"></i></a>
                                                        </c:if>
                                                    </td>
                                                </c:if>
                                            </tr>
...
<script src="${applicationScope['jsBaseUrl']}/javascript/event/eventMsgTemplate.js" defer onload="loadPageScript()"></script>
<script type="text/javascript">var eventId = "${eventMsgTemplate.eventId}";</script>
</body>
</html>
