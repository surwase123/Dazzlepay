                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">Industry Type</h5>
                            </div>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}"><i class="feather icon-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}/category/view">Industry Type</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <c:if test="${action != 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/category/add" var="actionUrl"/>
                            </c:if>
                            <c:if test="${action == 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/category/edit" var="actionUrl"/>
                            </c:if>
                            <form id="category-form" action="${actionUrl}" method="post">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label class="form-label">Industry Type Code</label>
                                            <input type="text" class="form-control" name="categoryCode" id="categoryCode" placeholder="Industry Type Code" value="${category.categoryCode}" <c:if test="${action == 'update'}">readonly</c:if>>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label class="form-label">Industry Type Name</label>
                                            <input type="text" class="form-control" name="categoryName" id="categoryName" placeholder="Industry Type Name" value="${category.categoryName}">
                                        </div>
                                    </div>
                                    <div class="col-md-12">
	                        		<div class="form-group">
	                            		<label class="form-label">Industry Type Description</label>
	                            		<textarea name="description" id="description" class="width">${category.description}</textarea>
	                       			</div>
                   					</div>
                                    ...
                                    <input type="hidden" name="id" value="<c:if test='${!empty(category.id)}'>${category.id}</c:if><c:if test='${empty(category.id)}'>0</c:if>">
                                                ...
                                                <c:if test="${pageRole.isUpdate == 1 || pageRole.isDelete == 1}">
                                                    <td>
                                                        <c:if test="${pageRole.isUpdate == 1}">
                                                            <a href="javascript:updateCategory('${category.id}')"><i class="feather icon-edit"></i></a>
                                                        </c:if>
                                                        <c:if test="${pageRole.isDelete == 1}">
                                                            <a href="javascript:deleteCategory('${category.id}', '${sessionScope.loginDTO.loginId}')"><i class="feather icon-trash-2"></i></a>
                                                        </c:if>
                                                    </td>
                                                </c:if>
                                            </tr>
...
<script src="${applicationScope['jsBaseUrl']}/javascript/category.js" defer onload="loadPageScript()"></script>
</body>
</html>
