        ...
        <div class="pcoded-content">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">System Parameter</h5>
                            </div>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}"><i class="feather icon-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}/sysParameter/view">System Parameter</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <c:if test="${action != 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/sysParameter/add" var="actionUrl"/>
                            </c:if>
                            <c:if test="${action == 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/sysParameter/edit" var="actionUrl"/>
                            </c:if>
                            <form id="sysParam-form" action="${actionUrl}" method="post">
								<div class="col-md-12">
                                     <div class="form-group">
                                         <label class="form-label">Parameter Name</label>
                                         <input type="text" class="form-control" name="paramName" id="paramName" placeholder="Parameter Name" value="${sysParameter.paramName}" <c:if test="${action == 'update'}">readonly</c:if>>
                                     </div>
                                </div>
                                <div class="col-md-12">
                                     <div class="form-group">
                                         <label class="form-label">Parameter Display Name</label>
                                         <input type="text" class="form-control" name="paramDisplayName" id="paramDisplayName" placeholder="Parameter Display Name" value="${sysParameter.paramDisplayName}">
                                     </div>
                                </div>
                                <div class="col-md-12">
                                     <div class="form-group">
                                         <label class="form-label">Parameter Value</label>
                                         <input type="text" class="form-control" name="paramValue" id="paramValue" placeholder="Parameter Value" value="${sysParameter.paramValue}">
                                     </div>
                                </div>
                                ...
                                <input type="hidden" name="id" value="<c:if test='${!empty(sysParameter.id)}'>${sysParameter.id}</c:if><c:if test='${empty(sysParameter.id)}'>0</c:if>">
					...
					<c:if test="${!empty(sysParamList)}">
                    <div class="card">
                        <div class="card-header">
                            <h5>Recon Visa Transaction Type</h5>
                        </div>
                        <div class="card-body">
                            <div class="dt-responsive table-responsive">
                                <table id="multi-colum-dt" class="table table-striped table-bordered nowrap">
                                    ...
                                    <tbody>
                                         <c:forEach items="${sysParamList}" var="sysParam" varStatus="index">
                                            <tr>
                                                <td>${index.count}</td>
                                                <td>${sysParam.paramName}</td>
                                                <td>${sysParam.paramDisplayName}</td>
                                                <td>${sysParam.paramValue}</td>
                                                <c:if test="${pageRole.isUpdate == 1 || pageRole.isDelete == 1}">
                                                    <td>
                                                        <c:if test="${pageRole.isUpdate == 1}">
                                                            <a href="javascript:updateSystemParam('${sysParam.paramName}', '${sessionScope.loginDTO.loginId}')"><i class="feather icon-edit"></i></a>
                                                        </c:if>
                                                        <c:if test="${pageRole.isDelete == 1}">
                                                            <a href="javascript:deleteSystemParam('${sysParam.id}', '${sessionScope.loginDTO.loginId}')"><i class="feather icon-trash-2"></i></a>
                                                        </c:if>
                                                    </td>
                                                </c:if>
                                            </tr>
...
<script src="${applicationScope['jsBaseUrl']}/javascript/systemParameter.js" defer onload="loadPageScript()"></script>
</body>
</html>
