                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">Merchant Version</h5>
                            </div>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}"><i class="feather icon-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}/merchantPlan/view">Merchant Version</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <c:if test="${action != 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/merchantPlan/add" var="actionUrl"/>
                            </c:if>
                            <c:if test="${action == 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/merchantPlan/edit" var="actionUrl"/>
                            </c:if>
                            <form id="merchantVersion-form" action="${actionUrl}" method="post">
                                     <div class="col-md-12">
                                        <div class="form-group">
                                            <label class="form-label">Version Name</label>
                                            <input type="text" class="form-control" name="versionName" id="versionName" value="${merchantVersion.versionName}" <c:if test="${action == 'update'}">readonly</c:if>>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label class="form-label">Description</label>
                                            <textarea name="description" id="description" class="width">${merchantVersion.description}</textarea>
                                        </div>
                                       </div>
                                    ...
                                    <input type="hidden" name="id" value="<c:if test='${!empty(merchantVersion.id)}'>${merchantVersion.id}</c:if><c:if test='${empty(merchantVersion.id)}'>0</c:if>">
                     ...
                     <c:if test="${!empty(merchantVersionList)}">
                    <div class="card">
                        <div class="card-header">
                            <h5>Merchant Version</h5>
                        </div>
                                ...
                                <table id="multi-colum-dt" class="table table-striped table-bordered nowrap">
                                    ...
                                    <tbody>
                                         <c:forEach items="${merchantVersionList}" var="merchantVersion" varStatus="index">
                                            <tr>
                                                <td>${index.count}</td>
                                                <td>${merchantVersion.versionName}</td>
                                                <td>${merchantVersion.description}</td>
                                              
                                                <c:if test="${pageRole.isUpdate == 1 || pageRole.isDelete == 1}">
                                                    <td>
                                                        <c:if test="${pageRole.isUpdate == 1}">
                                                            <a href="javascript:updateMerchantVersion('${merchantVersion.id}')"><i class="feather icon-edit"></i></a>
                                                        </c:if>
                                                        <c:if test="${pageRole.isDelete == 1}">
                                                            <a href="javascript:deleteMerchantVersion('${merchantVersion.id}', '${sessionScope.loginDTO.loginId}')"><i class="feather icon-trash-2"></i></a>
                                                        </c:if>
                                                    </td>
                                                </c:if>
                                            </tr>
...
<script src="${applicationScope['jsBaseUrl']}/javascript/merchantVersion.js" defer onload="loadPageScript()"></script>
</body>
</html>
