                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">Entity</h5>
                            </div>
                            <ul class="breadcrumb">
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}"><i class="feather icon-home"></i></a></li>
                                <li class="breadcrumb-item"><a href="${applicationScope['baseUrl']}/entity/view">Entity</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <c:if test="${action != 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/entity/add" var="actionUrl"/>
                            </c:if>
                            <c:if test="${action == 'update'}">
                                <c:set value="${applicationScope['baseUrl']}/entity/edit" var="actionUrl"/>
                            </c:if>
                            <form id="entity-form" action="${actionUrl}" method="post">
	                                <div class="col-md-12">
	                                    <div class="form-group">
	                                        <label class="form-label">Table Name</label>
	                                        <select class="form-control js-example-basic-single" name="tableName" id="tableName">
	                                            <option value="">--Table Name--</option>
	                                            <c:forEach items="${tablesList}" var="table" varStatus="index">
	                                                  <option value="${table}"<c:if test="${table == entity.tableName}">selected</c:if>>${table}</option>
	                                            </c:forEach>
	                                        </select>
	                                    </div>
	                                </div>
	                                <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="entityName" id="entityName" value="${entity.entityName}">
                                        </div>
                                    </div>
	                                <div class="col-md-12">
				                        <div class="form-group">
				                            <label class="form-label">Description</label>
				                            <textarea name="description" id="description" class="width">${(entity.description)}</textarea>
				                        </div>
                    				</div>
                                    ...
                                    <input type="hidden" name="id" value="<c:if test='${!empty(entity.id)}'>${entity.id}</c:if><c:if test='${empty(entity.id)}'>0</c:if>">
                                                ...
                                                <c:if test="${pageRole.isUpdate == 1 || pageRole.isDelete == 1}">
                                                    <td>
                                                        <c:if test="${pageRole.isUpdate == 1}">
                                                            <a href="javascript:updateEntity('${entity.id}')"><i class="feather icon-edit"></i></a>
                                                        </c:if>
                                                        <c:if test="${pageRole.isDelete == 1}">
                                                            <a href="javascript:deleteEntity('${entity.id}', '${sessionScope.loginDTO.loginId}')"><i class="feather icon-trash-2"></i></a>
                                                        </c:if>
                                                    </td>
                                                </c:if>
                                            </tr>
...
<script src="${applicationScope['jsBaseUrl']}/javascript/event/entity.js" defer onload="loadPageScript()"></script>
</body>
</html>
